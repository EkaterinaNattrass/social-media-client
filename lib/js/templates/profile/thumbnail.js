...
exports.profileThumbnail = void 0;
const profileThumbnail = profile => {
  const element = document.createElement('a');
  element.classList.add('profile', 'thumbnail');
  element.href = "./?view=profile&name=".concat(profile.name);
  const img = new Image();
  ...
  element.title = "".concat(profile.name, "'s Profile");
  element.append(img);
  return element;
};
exports.profileThumbnail = profileThumbnail;